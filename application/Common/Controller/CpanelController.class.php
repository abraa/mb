...
//            $result = true;
        ...
        if($result !== false) {
            ...
            //操作日志
            if($insert){
                $log_msg = L('ADD') . L('SUCCESS');
            }else{
                $log_msg = L('EDIT') . L('SUCCESS');
            }
            $this->logAdmin($log_msg, 0, array('params'=>$pk.'='.$params[$pk]));

            $this->success($msg, '', true);
        }else {
            $this->logAdmin(L('SAVE').L('ERROR'), 0, array('params'=>$pk.'='.$params[$pk]));  //操作失败日志
    ...
    public function delete() {
        ...
        if($this->dbModel->where($where)->delete()) {
            ...
            $this->logAdmin(L('DELETE') . L('SUCCESS'), 0, array('params'=>$log_param));    //操作日志
            $this->success(L('DELETE') . L('SUCCESS'));
        }else {
            $this->logAdmin(L('DELETE') . L('ERROR'), 0, array('params'=>$itemId)); //操作日志
	...
	*/
     ...
     */
    protected function logAdmin($message, $user_id = 0, $merge_data = array()){
        D('Cpanel/AdminLog')->addLog($message, $user_id, $merge_data);
    }
}